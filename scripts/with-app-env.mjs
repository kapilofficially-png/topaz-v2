...
function main(argv) {
  ...
  // If running a local binary like vite and node_modules is missing, guide the user
  if (!existsSync(join(root, "node_modules"))) {
    console.error("\n=========================================================");
    console.error(`[ERROR] Missing dependencies! Cannot find '${command}'.`);
    console.error("The 'node_modules' folder is not installed yet.");
    console.error("\nPlease run this command in your terminal first:");
    console.error("    npm install\n");
    console.error("Once installation finishes, start the app with:");
  ...
  if (isWindows) {
    ...
    const candidates = [
      ...
      join(binDir, `${command}.bat`),
      join(binDir, `${command}.exe`),
      join(binDir, command),
    ];
  ...
  const child = isWindows
    ? spawn(process.env.ComSpec || "cmd.exe", ["/d", "/s", "/c", targetCmd, ...args], { stdio: "inherit", env })
    : spawn(targetCmd, args, { stdio: "inherit", env });
